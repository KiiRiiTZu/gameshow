...
export function captureRankingMove(game, pool, board) {
  ...
  const movingBackToPool = Boolean(game.lastResult && !game.lastResult.correct);
  if (!movingIntoList && !movingBackToPool) return null;

  const itemId = movingIntoList ? game.proposal.itemId : game.lastResult.itemId;
  const source = movingIntoList
  ...
  return {
    itemId,
    direction: movingIntoList ? "into-list" : "back-to-pool",
    team: movingIntoList ? game.proposal.team : game.lastResult.team,
...
export function playRankingMove(move, pool, board) {
  ...
  token.className = `ranking-moving-token ${move.team}`;
